from src.slow.layers import Conv, AvgPool
import numpy as np
import torch 
...
def test_conv():
    ...
    assert np.allclose(conv_cnn.W['grad'], conv_pt.weight.grad.numpy(), atol=1e-7) # 1e-7 tolerance.
    assert np.allclose(conv_cnn.b['grad'], conv_pt.bias.grad.numpy(), atol=1e-7) # 1e-7 tolerance.
 
@register_test
def test_avgpool():
    n, c, h, w = 10, 6, 4, 4 # Image.
